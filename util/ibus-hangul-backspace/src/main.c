...
start_component (void)
{
    ...
    component = ibus_component_new ("org.freedesktop.IBus.HangulBackspace",
                                    "hangul-backspace",
                                    "0.1.0",
                                    "GPL",
    ...
    ibus_component_add_engine (component,
                               ibus_engine_desc_new ("hangul-backspace",
                                                     "hangul-backspace",
                                                     "hangul-backspace",
                                                     "ko",
                                                     "GPL",
                                                     "Peng Huang <shawn.p.huang@gmail.com>",
                                                     PKGDATADIR"/icons/hangul-backspace.svg",
                                                     "us"));

    factory = ibus_factory_new (ibus_bus_get_connection (bus));

    ibus_factory_add_engine (factory, "hangul-backspace", IBUS_TYPE_HANGUL_ENGINE);

    if (ibus) {
        ibus_bus_request_name (bus, "org.freedesktop.IBus.HangulBackspace", 0);
    }
    else {
