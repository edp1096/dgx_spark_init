> Version 1.0.1 adds `nvfancontrol --speed RPM` (1890–13500), a one-time RPM-floor request. See [speed usage](docs/speed.md). The separate [C auto/max controller](docs/watcher.md) is optional.

## Installation
* Run below & reboot
```sh
cd dist
sudo dpkg -i ./nvfancontrol_1.0.1_arm64.deb
```

## Usage
```sh
cd dist
sudo ./nvfanwatch --profile 2
```

## Service
Profile 2 at boot (from the project directory):
```sh
sudo install -m 755 dist/nvfanwatch /usr/local/sbin/nvfanwatch
sudo nvfanwatch --stop
sudo systemctl disable --now nvfancontrol.service
sudo tee /etc/systemd/system/nvfanwatch.service >/dev/null <<'EOF'
[Unit]
Description=DGX Spark fan control
[Service]
ExecStart=/usr/local/sbin/nvfanwatch --profile 2 --foreground
[Install]
WantedBy=multi-user.target
EOF
sudo systemctl daemon-reload
sudo systemctl enable --now nvfanwatch
```
