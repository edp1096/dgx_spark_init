...
test('Qwen 3.8 EXL3 exposes native effort and preserves legacy on', () => {
  assert.deepEqual(modelCapabilities('qwen3.8-exl3'), {
    family: 'qwen3.8-exl3', reasoning: 'effort', reasoningLevels: ['none', 'low', 'medium', 'xhigh'],
  });
  assert.equal(normalizeReasoningEffort('qwen3.8-exl3', 'none'), 'none');
  for (const level of ['low', 'medium', 'xhigh']) assert.equal(normalizeReasoningEffort('qwen3.8-exl3', level), level);
  for (const level of ['on', 'high']) assert.equal(normalizeReasoningEffort('qwen3.8-exl3', level), 'xhigh');
  for (const level of ['', 'off', 'false']) assert.equal(normalizeReasoningEffort('qwen3.8-exl3', level), 'none');
  assert.equal(reasoningEffortLabel('xhigh', 'qwen3.8-exl3'), '매우 높음');
});
