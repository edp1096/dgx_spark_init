...
test('classifies attachments and limits inline video previews to browser-friendly formats', () => {
  assert.equal(attachmentKind({ name: 'report.hwp', mime: 'application/x-hwp' }), 'document');
  assert.equal(attachmentKind({ name: 'report.hwpx', mime: 'application/vnd.hancom.hwpx' }), 'document');
