...
export function normalizeReasoningEffort(modelType, value) {
  ...
  if (profile.family === 'qwen3.8-exl3') {
    if (THINKING_OFF_VALUES.has(normalized)) return 'none';
    return profile.reasoningLevels.includes(normalized) ? normalized : 'xhigh';
  }
...
export function reasoningEffortLabel(value, modelType) {
  if (modelType === 'qwen3.8-exl3') {
    return { none: '꺼짐', low: '낮음', medium: '중간', xhigh: '매우 높음' }[value] || String(value || '');
  }
  switch (value) {
    case 'off':
    case 'none': return '꺼짐';
