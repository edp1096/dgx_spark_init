...
export function attachmentKind(attachment) {
  ...
  if (/\.(mp3|wav)$/i.test(name) || (/\.ogg$/i.test(name) && !mime.startsWith('video/'))) return 'audio';
	if (/\.(zip|pdf|md|markdown|html?|csv|tsv|json|xml|ya?ml|toml|js|jsx|ts|tsx|css|scss|py|go|rs|java|c|h|cpp|hpp|cs|sh|ps1|sql|ini|conf|log|docx|pptx|xlsx|odt|odp|ods|epub|hwp|hwpx)$/i.test(name)) return 'document';
  return 'video';
}
