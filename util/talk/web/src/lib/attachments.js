...
export function attachmentKind(attachment) {
  ...
  if (mime.startsWith('video/')) return 'video';
	if (mime === 'application/pdf' || mime.startsWith('text/') || mime.includes('document') || mime.includes('presentation') || mime.includes('spreadsheet') || mime.includes('opendocument') || mime.includes('epub') || mime.includes('hwp') || ['application/json', 'application/xml', 'application/yaml'].includes(mime)) return 'document';
  const name = attachment?.name || '';
  if (/\.(png|jpe?g|webp)$/i.test(name)) return 'image';
  if (/\.(mp3|wav)$/i.test(name) || (/\.ogg$/i.test(name) && !mime.startsWith('video/'))) return 'audio';
	if (/\.(pdf|txt|md|markdown|html?|csv|tsv|json|xml|ya?ml|toml|js|jsx|ts|tsx|css|scss|py|go|rs|java|c|h|cpp|hpp|cs|sh|ps1|sql|ini|conf|log|docx|pptx|xlsx|odt|odp|ods|epub|hwp|hwpx)$/i.test(name)) return 'document';
  return 'video';
}
