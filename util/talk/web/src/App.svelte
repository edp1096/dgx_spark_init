<script>
 ...
 const contextUsageBySession = new Map();
  ...
  $: shellColumns = artifactOpen && !libraryOpen
    ? (sidebarVisible ? `${sidebarWidth}px minmax(420px, 1fr) ${artifactPanelWidth}px` : `minmax(420px, 1fr) ${artifactPanelWidth}px`)
    : (sidebarVisible ? `${sidebarWidth}px 1fr` : '1fr');

  onMount(() => {
    const visualViewport = window.visualViewport;
    const syncViewportHeight = () => {
      mobileViewport = window.matchMedia('(max-width: 600px)').matches;
  ...
  function openLibrary(tab = 'memory') {
    libraryVisit += 1;
    libraryTab = typeof tab === 'string' ? tab : 'memory';
    settingsOpen = false;
    libraryOpen = true;
    artifactOpen = false;
    selectedArtifactId = '';
  ...
  function publishMessages(sessionId, nextMessages) {
    // Saving gives optimistic messages database IDs. Keep their DOM identity so
    // the browser retains the reading anchor and expanded content.
    const previous = chatController.getMessages(sessionId);
    if (nextMessages !== previous) {
      const byId = new Map(previous.filter(message => message.id).map(message => [message.id, message]));
      nextMessages.forEach((message, index) => {
        const old = byId.get(message.id) || (!previous[index]?.id && previous[index]?.role === message.role ? previous[index] : null);
        if (old) message.render_key = old.render_key || (old.id ? `id:${old.id}` : `pending:${sessionId}:${index}:${old.role || ''}`);
      });
    }
...
<div class="shell" class:artifact-open={artifactOpen && !libraryOpen} style:grid-template-columns={shellColumns}>
  ...
  {#if libraryOpen}
    <main class="library-main">
      {#key libraryVisit}<KnowledgeHub initialTab={libraryTab} {health} onclose={closeLibrary} />{/key}
    </main>
  {:else}
  <main>
    <ChatHeader
    ...
    <WorkflowRuns sessionId={activeId} version={workflowVersion} {running} onResume={(id)=>{input=`@resume:${id}\n${input || '이어서 진행해.'}`;}} />
    <Composer
      onManageWorkflows={() => openLibrary('workflows')}
      {pendingAttachments}
...
{#if settingsOpen && settings}
  <SettingsModal
    ...
    onManageSkills={() => openLibrary('skills')}
  />
{/if}
