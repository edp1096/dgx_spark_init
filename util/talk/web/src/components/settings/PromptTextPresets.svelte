<script>
  ...
  let filter = '';
  let editingDirect = !model.system_prompt_preset && Boolean(model.system_prompt);
  $: selectedSource = model.system_prompt_preset ? `preset:${model.system_prompt_preset}` : editingDirect || model.system_prompt ? 'custom' : 'none';

  function selectPreset(name) {
    const preset = model.system_prompt_presets.find((item) => item.name === name);
    if (!preset) return;
    editingDirect = false;
    model = { ...model, system_prompt_preset: name, system_prompt: preset.prompt };
  }

  function selectSource(value) {
    if (value.startsWith('preset:')) return selectPreset(value.slice(7));
    editingDirect = value === 'custom';
    model = { ...model, system_prompt_preset: '', system_prompt: '' };
  }

  function editCurrentText() {
    editingDirect = true;
    model = { ...model, system_prompt_preset: '' };
  }
...
<fieldset class="prompt-presets">
  ...
  {#if selectedSource === 'none'}
    <small>사용자 지정 시스템 프롬프트를 추가하지 않습니다.</small>
  {:else}
    <label>시스템 프롬프트 내용<textarea class="system-prompt" bind:value={model.system_prompt} rows="6" placeholder="예: 모든 답변은 한국어 존댓말로 작성한다."></textarea></label>
  {/if}
  {#if model.system_prompt_preset}
    <button type="button" onclick={editCurrentText}>현재 내용으로 직접 편집</button>
  {/if}
