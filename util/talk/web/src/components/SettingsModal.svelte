      ...
      <div id="settings-panel-chat" class="settings-tab-panel" class:active={activeTab === 'chat'} role="tabpanel" aria-labelledby="settings-tab-chat">
        ...
        <fieldset>
          ...
          {#if modelProfile.reasoning === 'toggle'}
            <div class="settings-toggle-field"><span>기본 Thinking</span><button type="button" class:active={gemmaThinkingValue === 'on'} onclick={toggleDefaultThinking} aria-pressed={gemmaThinkingValue === 'on'}>{gemmaThinkingValue === 'on' ? 'Thinking 켜짐' : 'Thinking 꺼짐'}</button><small>{modelProfile.family === 'gemma4' ? 'Gemma 4는 단계별 effort를 지원하지 않습니다.' : 'Thinking을 켜거나 끕니다.'}</small></div>
          ...
          {:else if modelProfile.family === 'qwen3.8' || modelProfile.family === 'qwen3.8-exl3'}
            <label>기본 reasoning effort<select bind:value={settings.model.reasoning_effort} aria-label="기본 reasoning effort">{#each modelProfile.reasoningLevels as level}<option value={level}>{reasoningEffortLabel(level, settings.model.model_type)}</option>{/each}</select><small>Qwen3.8은 Thinking 꺼짐과 low·medium·xhigh 단계를 지원합니다.</small></label>
        ...
        <fieldset>
          ...
          <label>최대 출력 토큰 (생각 과정 포함)<input type="number" min="256" step="256" bind:value={settings.context.output_reserve} /><small>한 번의 모델 응답 상한입니다. 같은 크기의 공간을 문맥에서 확보합니다. 긴 코드가 잘리면 늘려 주세요.</small></label>
      ...
      <div id="settings-panel-features" class="settings-tab-panel" class:active={activeTab === 'features'} role="tabpanel" aria-labelledby="settings-tab-features">
        ...
        <div class="settings-section" hidden={featureSection !== 'documents'}>
          <fieldset><legend>문서 생성</legend>
            ...
            <small>보고서는 본문·표·첨부 이미지, 발표자료는 제목·목록을 지원합니다. 스프레드시트는 여러 시트·기본 수식·숫자 서식을 지원합니다. Office 파일과 같은 내용의 PDF도 생성합니다. 원본과 배치가 다를 수 있으며 PDF 생성 실패 시 원본만 반환합니다. 서비스 주소와 실행 상태는 시스템의 지원 서비스에서 관리합니다.</small>
          </fieldset>
        </div>
        ...
        <fieldset>
          ...
          <label>최대 호출 라운드 (≤ 64)<input type="number" min="1" max="64" bind:value={settings.tools.max_rounds} /></label>
          <small>기본 24 · 최대 64. 한 라운드에 여러 도구를 호출할 수 있으며, 작업 절차에서는 단계마다 적용됩니다.</small>
          <label>검색 결과 수 (≤ 30)<input type="number" min="1" max="30" bind:value={settings.tools.search_results} /></label>
          <small>검색 1회당 최대 건수입니다. 기본 15 · 최대 30이며, 검색엔진의 실제 결과 수에 따라 줄어듭니다.</small>
          <label>도구 타임아웃 (> 0초)<input bind:value={settings.tools.timeout} placeholder="15s" /></label>
          <small>0보다 큰 시간으로 입력합니다. 예: 15s(15초), 1m(1분). 기본 15초이며 웹 검색·페이지 읽기에 적용됩니다.</small>
        </fieldset>
        </div>
