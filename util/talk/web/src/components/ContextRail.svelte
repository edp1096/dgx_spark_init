  ...
  <aside class="context-panel">
    ...
    {#if state}
      ...
      <div class="context-stats">
        ...
        <span>스킬 절차 <strong>{state.skill_tokens?.toLocaleString() || 0}</strong></span>
      ...
      {#if state.skills?.length}<section class="context-recalls"><strong>{state.preview ? '직접 지정한 스킬' : '이번 요청에 불러온 스킬'}</strong>{#each state.skills as skill}<details><summary>{skill.name} · {skill.source === 'selected' ? '직접 지정' : '자동 선택'} · 약 {skill.tokens}토큰</summary><pre style="white-space:pre-wrap">{skill.instructions}</pre></details>{/each}</section>{/if}
