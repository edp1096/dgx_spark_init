...
export const getSkill = (name) => request(`/api/skills/${encodeURIComponent(name)}`);
export const saveSkill = (name, skill) => request(`/api/skills/${encodeURIComponent(name)}`, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(skill)});
export const deleteSkill = (name) => request(`/api/skills/${encodeURIComponent(name)}`, {method:'DELETE'});
