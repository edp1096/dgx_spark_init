...
test('persists DeepSeek recovery options independently in the selected set', async ({ page, request }) => {
  ...
  try {
    ...
    await selectOption(editor.getByLabel('편집할 세트'), 'ds4fve');
    ...
    await expectControlValue(card.getByLabel('도구 호출 형식 복구'), '1');
    await expectControlValue(card.getByLabel('반복 요청 캐시 보완'), '0');
    await selectOption(card.getByLabel('도구 호출 형식 복구'), '0');
    await selectOption(card.getByLabel('반복 요청 캐시 보완'), '1');
