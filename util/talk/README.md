...
XHigh의 단계형 effort를 사용합니다. Qwen EXL3도 꺼짐·낮음(low)·중간(medium)·매우 높음(xhigh)을 지원하며, 선택값을 모델의 채팅 템플릿에 직접 전달합니다. 기존 EXL3 설정의 `on`은 이전 동작과 같은 `xhigh`로 읽습니다.
...
예상 토큰을 조절할 수 있습니다. 기본값은 각각 자동 감지, 80%, 16384, 2048,
8192, 2048입니다. 모델이 문맥 초과를 반환하면 한 번 더 보수적으로 정리한
뒤 자동 재시도하고, 그래도 실패하면 기존 대화는 그대로 둔 채 오류를
표시합니다.

## 기억과 과거 대화 회수

좌측의 **라이브러리 > 기억**에서 기억을 하나의 목록으로 관리합니다. **호출 범위**는
...
좌측의 **라이브러리 > 지식**에서 보관함을 만들고 PDF, 일반 텍스트·마크다운·HTML·표·설정·소스,
...
상단의 **모델 메뉴**에서 모델과 추론 강도를 변경하고, **도구 메뉴**에서 웹검색·음성대기와
대화별 SSH 허용을 관리합니다. **연결 상태**에서는 모델과 Extra 서비스의 상태·실행을 확인합니다.
PC와 모바일이 같은 메뉴 구성을 사용합니다.

채팅 상단의 **도구 > 웹검색**을 **자동**으로 켜면 모델이 필요한 경우 DuckDuckGo 검색 또는
...
**설정 > 프로필 > AI 캐릭터**에서 AI 이름·소개·아바타·페르소나·말투를 설정합니다.
기본 아바타를 고르거나 이미지를 업로드할 수 있습니다. **프로필 > 내 프로필**의 이름·아바타는 별도로
저장하며, **시스템 > 외형**에서는 화면 테마를 고릅니다. AI 이름은 사이드바와 대화 메시지에 표시됩니다.
모델이나 페르소나를 바꿔도 이름·아바타와 공통 응답 조건은 유지됩니다.

**설정 > 프로필 > AI 캐릭터 > 프롬프트 작성 방식**에서 텍스트·프리셋 또는 페르소나·조건 조합을 선택합니다.
텍스트·프리셋의 **없음**은 내용을 비우고, **직접 작성**은 빈 내용으로 시작합니다.
기존 프리셋을 바탕으로 수정하려면 **현재 내용으로 직접 편집**을 사용합니다.
조합은 페르소나 0~1개와 추가 조건 여러 개로 구성합니다. 답변 길이·말투 등 같은 선택 제한에
속한 조건은 하나만 선택됩니다. 페르소나·조건을 추가·편집·삭제하고 선택 묶음을 이름으로
저장할 수 있으며, 삭제한 항목은 저장된 조합에서도 제외됩니다.

최종 프롬프트 미리보기는 실제 전역 지침과 같은 순서·내용입니다. 서버가 조합을 검증해
`model.system_prompt`에 반영하므로 모델 입력과 컨텍스트 예산 계산도 동일한 문자열을 사용합니다.
기존 직접 입력과 프리셋은 유지합니다. 조합에서 직접 입력으로 전환하면 완성된 문장을 편집할 수
있습니다. 변경 내용은 설정 저장 후 새 요청부터 적용되며, 도구 사용 허용·승인 설정은 바꾸지 않습니다.

**라이브러리 > 스킬**에서 개별 작업 지침을 추가·편집·삭제하거나 내장 스킬을 복사합니다.
기억은 사실, 지식은 참고 자료, 전역 시스템 프롬프트는 모든 대화의 기본 지침입니다.
스킬은 관련 작업에서만 모델이 불러옵니다. 입력창의 **＋ → 스킬·작업 절차**로 직접 지정할 수도 있으며,
선택은 해당 요청에만 적용됩니다. 필요한 도구가 꺼져 있으면 해당 스킬을 사용할 수 없습니다.
사용 기록은 답변에, 불러온 전문과 추정 토큰은 컨텍스트 지도에 표시됩니다.
스킬은 앱 DB에 저장하며, 내장 스킬 원문은 바이너리에 포함됩니다.
전체 사용 여부는 **설정 > 기능 > 스킬·기록**에서 조정합니다.
스킬은 도구의 실행 권한이나 승인 규칙을 변경하지 않습니다.

**라이브러리 > 작업 절차**에서는 여러 스킬을 단계로 묶습니다. 코딩·장애 진단·조사·미디어·
문서·이미지 절차를 복사해 순서, 완료 조건, 실행 근거와 재시도를 편집할 수 있습니다.
입력창의 **＋ → 스킬·작업 절차**에서 순서를 확인해 적용하거나 모델의 자동 선택을 사용합니다.
현재 단계의 지침과 이전 결과만 전달하며, 실행 근거가 필요한 단계는 실제 도구 결과를 확인합니다.
진행 기록은 대화 아래에 저장되고, 중단된 단계는 **이 작업 이어하기**로 재개합니다.
진행 중인 절차와 스킬 구성은 편집해도 기존 작업에 소급 적용되지 않습니다.
...
`unsafely-treat-insecure-origin-as-secure` 목록에 등록해야 합니다. **도구 > HTTP 마이크 설정 방법** 또는 **설정 > 음성 > 마이크 사용 방법**에서 브라우저 주소와 등록할 현재 화면 주소를 복사할 수 있습니다.
Chrome은 `chrome://flags/#unsafely-treat-insecure-origin-as-secure`, Edge는 `edge://flags/#unsafely-treat-insecure-origin-as-secure`를 주소창에 붙여넣고, 현재 화면의 origin(스킴·호스트·포트)을 추가한 뒤 Enabled와 Relaunch/Restart를 선택합니다. 이 예외 설정은 HTTP 통신을 암호화하지 않습니다. 주소창의 사이트
권한에서도 마이크를 허용해야 합니다.

PC·모바일 모두 상단 **도구 > 음성대기**를 켜면 마이크 스트림을 유지하고
...
**설정 > 기능 > 문서**에서 DOCX·PPTX·XLSX·PDF 생성을 켜고 문서 서비스 주소를 지정합니다.
시스템의 서비스 구성에서 **Extra Documents**를 원하는 세트·호스트에 추가할 수 있습니다.
결과 카드에서 원본을 내려받고 같은 내용으로 별도 생성한 PDF를 모달로 확인합니다. PDF와 원본의 배치는 다를 수 있습니다. 현재 보고서는 본문·표,
발표자료는 제목·목록, 스프레드시트는 여러 시트·기본 수식을 지원합니다. 운영에 LibreOffice는 필요하지 않습니다. HWP/HWPX와 이미지 삽입은 아직 제공하지 않습니다.
...
설정 → 시스템 → AI 세트 → DeepSeek 서비스 → 실행·포트 상세 설정에서
`도구 호출 형식 복구`(기본 켬)와 `반복 요청 캐시 보완`(기본 끔)을 각각 설정한다.
저장한 값은 앱에 내장된 레시피의 두 rank에 전달되며, 적용하려면 서비스를 다시 시작한다.
[2026-09-09 비교](../../compose_yaml/ds4fve_vllm/bench/reports/2026-09-09/README.md)에서는
DSML 단독 적용의 18개 응답이 기준과 같았고, SWA 보완은 짧은 요청에 추가 지연이 있었다.
이는 128K 상한에서 순차 실행한 작은 텍스트·도구 표본이며 1M·동시 부하 검증은 아니다.
