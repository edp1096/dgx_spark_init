	...
	for _, item := range items {
		...
		case db.MessageFailed, db.MessageCancelled:
			if item.Role == "assistant" {
				// Preserve code formatting so a later "continue" can resume it.
				// Do not replay incomplete tool calls or failed media inputs.
				if item.Content != "" {
					partial := "[중단 전 생성된 불완전한 답변]\n" + item.Content
					if len(out) > 0 && out[len(out)-1].Role == "assistant" {
						out[len(out)-1].Content += "\n\n" + partial
					} else {
						out = append(out, db.Message{ID: item.ID, SessionID: item.SessionID, Role: "assistant", Status: db.MessageCompleted, Content: partial})
					}
				}
				continue
			}
