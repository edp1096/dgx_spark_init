...
func (s *Server) sshKeys(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		...
		keys, err := s.sshSnapshot().Keys(r.Context())
		if err != nil {
	...
	key, err := s.sshSnapshot().ImportKey(r.Context(), keyID, data)
	if err != nil {
		writeExtraError(w, err)
		return
	}
...
func (s *Server) sshKey(w http.ResponseWriter, r *http.Request) {
	...
	if rest == "generate" {
		...
		key, err := s.sshSnapshot().GenerateKey(r.Context(), strings.TrimSpace(request.KeyID))
		if err != nil {
			writeExtraError(w, err)
			return
		}
	...
	if err := s.sshSnapshot().DeleteKey(r.Context(), rest); err != nil {
		writeExtraError(w, err)
		return
	}
