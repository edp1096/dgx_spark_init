...
func TestCompletionLoopHidesToolProtocolAfterRoundLimit(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "도구 실행 한도") {
		t.Fatalf("missing round-limit notice: %v", err)
	}
...
func TestTruncatedToolCallIsNotExecutedOrRetried(t *testing.T) {
	calls := 0
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		fmt.Fprintln(w, `data: {"choices":[{"delta":{"reasoning_content":"partial","tool_calls":[{"index":0,"id":"bad","type":"function","function":{"name":"web_search","arguments":"{\"query\":"}}]},"finish_reason":"length"}]}`)
		fmt.Fprintln(w, "data: [DONE]")
	}))
	defer backend.Close()
	result, err := runCompletionLoop(context.Background(), llm.New(backend.URL, "test", ""), []llm.Message{{Role: "user", Content: "test"}}, "test", "none", "", config.ToolsConfig{Enabled: true, MaxRounds: 3}, true, func(string, any) error { return nil })
	if err == nil || !strings.Contains(err.Error(), "finish_reason=length") || calls != 1 || len(result.ToolTrace) != 0 || result.Reasoning != "partial" {
		t.Fatalf("truncated call continued: calls=%d result=%+v err=%v", calls, result, err)
	}
}
