...
func (s *Server) runtimeAction(w http.ResponseWriter, r *http.Request) {
	...
	needsWorker := false
	if parts[0] == "bundles" {
		for _, c := range s.runtime.Catalog().ModelComponents(parts[1]) {
			needsWorker = needsWorker || (c.AutoAddress && (c.Host == "worker" || c.WorkerHost == "worker"))
		}
	} else if c, ok := s.runtime.Catalog().ResolveSupport(cfg.Runtime.ActiveBundle, parts[1]); ok {
		needsWorker = c.AutoAddress && (c.Host == "worker" || c.WorkerHost == "worker")
	}
	if needsWorker && (parts[2] == "start" || parts[2] == "restart" || parts[2] == "prepare") {
