...
func (s *Server) buildContext(ctx context.Context, sessionID string, items []db.Message, model string, cfg config.Config, client *llm.Client, force, preview bool) ([]llm.Message, contextState, error) {
	...
	registry := newCompletionToolRegistry(s, sessionID, cfg.Tools, contextToolsEnabled(ctx, cfg.Tools.Enabled), func(db.Attachment) error { return nil })
	_, skillErr := registry.selectSkills(context.Background(), messages)
	if skillErr != nil {
		return nil, state, skillErr
	}
	for _, name := range requestedSkills(messages) {
		item := registry.skills[name]
		state.Skills = append(state.Skills, skillUsage{Name: name, Source: "selected", Instructions: item.Instructions, Tokens: estimateTextTokens(item.Instructions)})
