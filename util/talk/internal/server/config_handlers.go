...
func (s *Server) configuration(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut:
		...
		if next.ASR.FFmpegEndpoint != old.ASR.FFmpegEndpoint && next.Extra.MediaEndpoint == old.Extra.MediaEndpoint {
			next.Extra.MediaEndpoint = next.ASR.FFmpegEndpoint
		}
		...
		if next.Runtime.Mode == "managed" && next.Runtime.Catalog != nil && next.Runtime.Catalog.Network != nil && next.Runtime.Catalog.Network.Enabled && (old.Runtime.Catalog == nil || !reflect.DeepEqual(old.Runtime.Catalog.Network, next.Runtime.Catalog.Network)) {
...
func (s *Server) replaceConfig(next config.Config) {
	...
	s.sshClient = supportssh.New(next.Extra.SSHEndpoint)
