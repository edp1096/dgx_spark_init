...
func TestDocumentServiceMigrationPreservesUserRemoval(t *testing.T) {
	cfg := Config{}
	cfg.Normalize()
	has := func() bool {
		for _, c := range cfg.Runtime.Catalog.Components {
			if c.ID == "extra-documents" {
				return true
			}
		}
		return false
	...
	if !has() {
		t.Fatal("new optional service missing")
	}
	for i, c := range cfg.Runtime.Catalog.Components {
		if c.ID == "extra-documents" {
			cfg.Runtime.Catalog.Components = append(cfg.Runtime.Catalog.Components[:i], cfg.Runtime.Catalog.Components[i+1:]...)
			break
		}
	}
	cfg.Normalize()
	if has() {
		t.Fatal("user-removed definition restored unexpectedly")
	}
}
