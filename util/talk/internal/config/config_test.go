...
func TestNormalizeConstrainsModelSpecificReasoning(t *testing.T) {
	for _, test := range []struct {
		...
		{"qwen3.8-exl3", "low", "low"},
		{"qwen3.8-exl3", "medium", "medium"},
		{"qwen3.8-exl3", "xhigh", "xhigh"},
		{"qwen3.8-exl3", "on", "xhigh"},
		{"qwen3.8-exl3", "off", "none"},
		{"gemma4", "xhigh", "on"},
		{"gemma4", "none", "none"},
		{"glm5.3", "none", "off"},
		{"glm5.3", "low", "low"},
...
func TestExpandedToolLimits(t *testing.T) {
	for _, tc := range []struct{ rounds, results, wantRounds, wantResults int }{{24, 15, 24, 15}, {64, 30, 64, 30}, {100, 100, 64, 30}, {8, 5, 8, 5}, {0, 0, 24, 15}} {
		...
		if c.Tools.MaxRounds != tc.wantRounds || c.Tools.SearchResults != tc.wantResults {
			t.Fatalf("unexpected limits: %+v -> %+v", tc, c.Tools)
		}
	}
}
