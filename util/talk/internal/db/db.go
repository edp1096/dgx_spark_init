...
func Open(path string) (*DB, error) {
 ...
 toolsets TEXT NOT NULL DEFAULT '[]', enabled INTEGER NOT NULL DEFAULT 1,
 updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
 );
 CREATE TABLE IF NOT EXISTS workflows(name TEXT PRIMARY KEY, data TEXT NOT NULL);
 CREATE TABLE IF NOT EXISTS workflow_runs(id TEXT PRIMARY KEY, session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, status TEXT NOT NULL, data TEXT NOT NULL, updated TEXT NOT NULL);
 CREATE INDEX IF NOT EXISTS workflow_runs_session ON workflow_runs(session_id,updated);
 UPDATE workflow_runs SET status='paused' WHERE status='running';
 CREATE TABLE IF NOT EXISTS builtin_skill_settings (name TEXT PRIMARY KEY, enabled INTEGER NOT NULL);
