...
func (c *Controller) UpdateCatalog(next Catalog, persist func() error) error {
	...
	for _, old := range c.catalog.Components {
		if !old.IsSupport() || old.Controller == "external" {
			continue
		}
		next, exists := validated.Component(old.ID)
		if exists && reflect.DeepEqual(old, next) && reflect.DeepEqual(c.catalog.Hosts[old.Host], validated.Hosts[next.Host]) {
			continue
		}
		ctx, cancel := context.WithTimeout(context.Background(), 6*time.Second)
		raw, inspectErr := executeHost(ctx, c.catalog.Hosts[old.Host], nil, "docker", "inspect", "-f", "{{.State.Running}}", old.Container)
		cancel()
		if inspectErr == nil && strings.TrimSpace(string(raw)) == "true" {
			return fmt.Errorf("%s 중지 후 지원 서비스 배치를 변경하세요", old.Name)
		}
	}
	if persist != nil {
...
func (c *Controller) startComponent(ctx context.Context, component Component) error {
	return c.prepareOrStartComponent(ctx, component, false)
}
func (c *Controller) prepareOrStartComponent(ctx context.Context, component Component, prepareOnly bool) error {
	...
	if image, ok := service["image"].(string); ok && embeddedBuildAsset(component.ComposeAsset) == "" {
