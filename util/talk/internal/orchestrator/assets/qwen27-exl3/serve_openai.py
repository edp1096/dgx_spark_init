...
SERVED_MODEL_NAME = os.environ.get("SERVED_MODEL_NAME", "qwen3.8-27b-exl3")
...
def generate_full(generator, tokenizer, messages, max_tokens, temperature,
                  ...
                  enable_thinking = True, on_text = None, reasoning_effort = "xhigh"):
...
def parse_request(body):
    ...
    return dict(
        ...
        model_id = body.get("model", SERVED_MODEL_NAME),
        enable_thinking = enable_thinking,
        reasoning_effort = template_kwargs.get("reasoning_effort", "xhigh"),
    ), None
    ...
    if not req["stream"]:
        try:
            text, calls, finish, ptoks, otoks, reasoning, content = await asyncio.to_thread(
                ...
                req["enable_thinking"], reasoning_effort=req["reasoning_effort"])
        ...
        def worker():
            try:
                text, calls, finish, ptoks, otoks, reasoning, content = generate_full(
                    ...
                    req["seed"], req["tools"], req["tool_choice"], req["stop"],
                    req["enable_thinking"], on_text = None if forced_choice else on_text, reasoning_effort=req["reasoning_effort"])
