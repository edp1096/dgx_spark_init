import {runs} from './blocks.mjs';
import {sheetExtras,validateSheetFeatures,applySheetFeatures,validateComputedCharts} from './spreadsheet-features.mjs';
...
const functions=new Set(['SUM','AVERAGE','COUNT','COUNTA','MIN','MAX','IF','AND','OR','NOT','ROUND','ROUNDUP','ROUNDDOWN','ABS','COUNTIF','SUMIF','IFERROR','VLOOKUP','XLOOKUP','SUMIFS','COUNTIFS']);
...
export function validateSheets(sheets){
 ...
 for(const s of sheets){
  object(s,['name','columns','rows','freeze_header','filter','orientation',...sheetExtras]);validateSheetFeatures(s);string(s.name,31);
  ...
  for(const row of s.rows){if(!Array.isArray(row)||row.length!==s.columns.length)throw Error('Each row must match the columns');for(const v of row){
   ...
   object(v,['formula','date','runs']);if(Object.keys(v).length!==1)throw Error('Cell must contain formula or date');
   if(v.runs!==undefined)runs(v.runs);
   else if(v.formula!==undefined)string(v.formula,512);
 ...
 if(sheets.every(s=>s.hidden))throw Error('At least one worksheet must be visible');
...
export async function renderSpreadsheet(input,dir){
 ...
 for(const s of input.sheets){
  ...
  for(const row of s.rows)ws.addRow(row.map(v=>v?.runs?{richText:runs(v.runs).map(r=>({text:r.text,font:{name:r.style?.font||FONT,size:r.style?.size||11,bold:r.style?.bold,italic:r.style?.italic,underline:r.style?.underline,color:{argb:'FF'+(r.style?.color||'202830')}}}))}:v?.date?new Date(v.date+'T00:00:00Z'):v?.formula?{formula:v.formula.replace(/^=/,'')}:v));
  ...
  await applySheetFeatures(workbook,ws,s);
 }
 ...
 validateComputedCharts(workbook,input);
 ...
 if(input.sheets.some(s=>s.charts?.length||s.pivots?.length||s.shapes?.length)){const spec=path.join(dir,'features.json');await fs.writeFile(spec,JSON.stringify(input));await exec(process.env.DOCUMENT_CALC_BIN||'/usr/local/bin/document-calc',['--decorate',output,spec],{timeout:60000,maxBuffer:8<<20});}
 if(input.sheets.some(s=>s.cells||s.merges||s.images||s.charts||s.pivots||s.shapes)){
  const {stdout:display}=await exec(process.env.DOCUMENT_CALC_BIN||'/usr/local/bin/document-calc',['--display',output],{timeout:60000,maxBuffer:8<<20});
  for(const row of JSON.parse(display)||[]){const ws=workbook.getWorksheet(row.sheet);if(ws)ws.getCell(row.cell)._documentDisplay=String(row.value);}
 }
