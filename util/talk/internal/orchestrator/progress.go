...
func inferProgress(component Component, logs string) progressInfo {
	...
	case "llama":
		if strings.Contains(logs, "listening on") {
			return progressInfo{Key: "api", Phase: "API 온라인", Detail: "GGUF API 준비 완료", Progress: 1}
		}
		if strings.Contains(logs, "loading model") {
			return progressInfo{Key: "weights", Phase: "GGUF 가중치 적재", Detail: "llama.cpp 모델과 MTP·비전 인코더를 준비합니다.", Progress: .4}
		}
		return progressInfo{Key: "container", Phase: "GGUF 서버 시작", Progress: .03}
	case "exl3":
...
func inferEXL3Progress(logs string) progressInfo {
	...
	if strings.Contains(logs, "== loading /models/target") {
		return progressInfo{Key: "engine", Phase: "EXL3 엔진 구성", Detail: "NVFP4 캐시와 MTP 구성을 적용합니다.", Progress: .10}
	}
