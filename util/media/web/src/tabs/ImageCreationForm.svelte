  ...
  {#if imageForm.mode === 'create'}
    <section class="krea-runtime-controls" aria-label="Krea 모델 내부 조정">
      ...
      <div class="runtime-control-row">
        <label><span>필터 완화</span><select disabled={kreaOptions.checkpoint !== 'official-int8' && kreaOptions.checkpoint !== 'official'} value={kreaOptions.filter_mode} onchange={(event) => { const mode = event.currentTarget.value; kreaOptions = { ...kreaOptions, filter_mode: mode, filter_strength: filterModeDefault(mode) } }}><option value="off">{kreaOptions.checkpoint === 'official-int8' || kreaOptions.checkpoint === 'official' ? '꺼짐 · 원본' : '체크포인트에 내장됨'}</option><option value="adherence">준수 강화 · skc3vo</option><option value="balanced">균형 · 2-vector</option><option value="strong">강함 · 3-vector</option></select></label>
