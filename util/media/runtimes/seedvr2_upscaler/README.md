...
SeedVR2 3B FP8을 독립 ComfyUI 런타임으로 실행하고 `POST /v1/images/upscale` 및
`POST /v1/videos/upscale` API를 제공합니다. 영상 API는 프레임레이트와 원음을 보존하고,
필요하면 `start_time`·`end_time`으로 최대 60초 구간만 처리합니다.
...
유지하는 2~4배 확대이며 어느 한 변도 4096px를 넘지 않도록 제한합니다.
