...
REFERENCE_SIZE = 1024
REFERENCE_BACKGROUND = (216, 216, 216)
...
def build_reference(rgb_image: Image.Image, mask_image: Image.Image) -> Image.Image:
    """Place only the selected garment pixels on a neutral square canvas."""
    ...
    padding = max(12, int(max(right - left, bottom - top) * 0.08))
    left = max(0, left - padding)
    top = max(0, top - padding)
    right = min(rgb_image.width, right + padding)
    bottom = min(rgb_image.height, bottom + padding)

    cropped_rgb = rgb_image.crop((left, top, right, bottom))
    cropped_mask = mask_image.crop((left, top, right, bottom))
    usable = REFERENCE_SIZE - 128
    scale = min(usable / cropped_rgb.width, usable / cropped_rgb.height)
    resized_size = (
        max(1, round(cropped_rgb.width * scale)),
        max(1, round(cropped_rgb.height * scale)),
    )
    cropped_rgb = cropped_rgb.resize(resized_size, Image.Resampling.LANCZOS)
    cropped_mask = cropped_mask.resize(resized_size, Image.Resampling.LANCZOS)

    reference = Image.new("RGB", (REFERENCE_SIZE, REFERENCE_SIZE), REFERENCE_BACKGROUND)
    position = (
        (REFERENCE_SIZE - resized_size[0]) // 2,
        (REFERENCE_SIZE - resized_size[1]) // 2,
    )
    reference.paste(cropped_rgb, position, cropped_mask)
    return reference
...
def segment(image: Image.Image, class_ids: tuple[int, ...], feather: float):
    ...
    clean_source = np.where(mask[:, :, None] > 0, source, 0).astype(np.uint8)
    rgba = np.dstack((clean_source, mask)).astype(np.uint8)
    reference = build_reference(rgb_image, mask_image)
    ...
    return Image.fromarray(rgba, mode="RGBA"), mask_image, reference, coverage, border_penalty, score
...
async def extract_garment(
    ...
    for index, upload in enumerate(images):
        ...
        try:
            ...
            image.load()
            cutout, mask, reference, coverage, border_penalty, score = segment(image, class_ids, feather)
            candidates.append({
                "index": index,
                "cutout": cutout,
                "mask": mask,
    ...
    return {
        ...
        "reference_b64": png_base64(chosen["reference"]),
        "reference_width": chosen["reference"].width,
        "reference_height": chosen["reference"].height,
        "reference_background": "#d8d8d8",
        "candidates": [
            {
                "index": item["index"],
