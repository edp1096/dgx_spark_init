...
func updateRequestContext(ctx context.Context, messages []llm.Message, tools []llm.Tool, refs map[string]int64, emit eventEmitter) ([]llm.Message, error) {
	...
	run.state.SystemToolTokens = max(0, run.state.EstimatedTokens-run.state.ActiveTokens-run.state.ToolResultTokens-run.state.SummaryTokens-run.state.RecallTokens)
