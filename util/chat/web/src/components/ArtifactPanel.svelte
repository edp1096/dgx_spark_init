<script>
  ...
  function download() {
    ...
    anchor.href = url;
    anchor.download = `${selected.title.replace(/[^\p{L}\p{N}._-]+/gu, '-') || 'artifact'}.html`;
    anchor.click();
...
<aside class="artifact-panel" aria-label="웹 생성물">
  ...
  <header class="artifact-header">
    ...
    <div class="artifact-header-actions">
      <button onclick={download} title="HTML 다운로드" aria-label="HTML 다운로드">
  ...
  <div class="artifact-stage">
    {#if selected}
      {#if mode === 'preview'}
        ...
        {/key}
      {:else}
        <pre><code>{selected.document}</code></pre>
      {/if}
    {:else}
