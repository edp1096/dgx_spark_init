...
def gather_dequant_many(requests: list) -> list[torch.Tensor]:
    ...
    for table, rel, owned in requests:
        ...
        w = torch.empty((uniq.numel(), table.dim), dtype=torch.uint8, device="cpu")
        s = torch.empty((uniq.numel(), table.sb), dtype=torch.uint8, device="cpu")
...
class ParallelEngramEmbedding(nn.Module):
    ...
    def lookup(
        ...
        if self.disk is not None:
            ids = indices.detach().to(device="cpu", dtype=torch.int64)
            count = ids.shape[0]
            heads = self.part_n_hash_cols
            local = ids[:, self.head_start:min(self.head_start+heads, self.n_hash_cols)]
            if local.shape[1] < heads:
                local = torch.cat((local, torch.full((count, heads-local.shape[1]), -1, dtype=torch.int64, device="cpu")), dim=1)
            ...
            out[:count].copy_(self.disk.gather_dequant(rel, owned).view(count, heads, self.dim))
            return
...
class Engram(nn.Module):
    ...
    def prepare_embeddings(self, hash_ids: torch.Tensor) -> None:
        ...
        """
        if getattr(self,'disk_prestaged',False): return
