...
def shared_scratch(plan):
    ...
    # and PER token shape. I/O outputs remain separately owned by each call.
...
class SlotLayer:
    ...
    def execution(self,m):
        m=next((n for n in (1,5,6,16,32,64,128,256,512) if n>=m),None)
        if m is None: raise ValueError("Expert execution capacity exceeds 512 tokens")
        key=(m,torch.cuda.current_stream().cuda_stream)
        ...
        ids=torch.zeros(m,self.topk,device='cuda',dtype=torch.int32)
        weights=torch.zeros(m,self.topk,device='cuda',dtype=torch.float32)
    ...
    def execute(self,x,weights,mapped,shape):
        if x.shape[0]>512:
            ...
            # qualified 512-token kernels and their fixed graph/scratch buffers.
            # Copy each result before the next replay overwrites its backing.
            output=torch.empty_like(x,dtype=torch.float32)
            topk=shape[1]
            for begin in range(0,x.shape[0],512):
                end=min(begin+512,x.shape[0])
...
def stats():
    ...
    return _stats|{'prefetch_pending':sum(len(c.prefetch_pending) for c in _layers.values()),
