...
class CudaGraphManager:
    def __init__(
        ...
        self.graphs: dict[BatchExecutionDescriptor, torch.cuda.CUDAGraph | BreakableCUDAGraphCapture] = {}
            ...
            for mode in [CUDAGraphMode.PIECEWISE, CUDAGraphMode.FULL]:
                ...
                for desc in descs:
                    ...
                    if (
                        ...
                        streamed_full = os.environ.get('DSV41_ATTENTION_GRAPHS') == '1'
                        if streamed_full:
                        ...
                        capture_context = graph if streamed_full else torch.cuda.graph(
                            graph, self.pool, stream=current_stream()
                        )
                        with capture_context:
                            forward_fn(CUDAGraphMode.FULL if streamed_full else CUDAGraphMode.NONE)
                        ...
                        if streamed_full:
                            logger.info('STREAMED_FULL_GRAPH %s %s', desc, graph)
    ...
    def dispatch(
        ...
        if os.environ.get('DSV41_BENCH_CONTROL') == '1':
            ...
            if self._graphs_captured and not cache_control.graphs_enabled:
    ...
    def run_fullgraph(self, desc: BatchExecutionDescriptor):
        ...
        short_graphs = getattr(self, 'short_context_graphs', None)
        if short_graphs:
            import streaming_graphs
            if streaming_graphs.runtime_context_upper_bound <= streaming_graphs.SHORT_CONTEXT_LIMIT:
                graph = short_graphs.get(desc, graph)
        graph.replay()
    ...
    def run_pw_graph(self, model: nn.Module, model_inputs: dict[str, Any]) -> Any:
        if self._graphs_captured and os.environ.get('DSV41_BENCH_CONTROL')=='1':
            import cache_control
            if not cache_control.graphs_enabled:
...
class ModelCudaGraphManager(CudaGraphManager):
    ...
    def capture(
        ...
        if (os.environ.get('DSV41_SHORT_CONTEXT_GRAPHS') == '1'
            ...
            import streaming_graphs
            normal_graphs = self.graphs
            self.graphs = {}
            streaming_graphs.capture_context_limit = streaming_graphs.SHORT_CONTEXT_LIMIT
            try:
                super().capture(create_forward_fn, progress_bar_desc + ' (context <= 512)')
                self.short_context_graphs = self.graphs
            finally:
                self.graphs = normal_graphs
                streaming_graphs.capture_context_limit = None
