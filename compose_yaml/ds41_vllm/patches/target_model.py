...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if getattr(self.ffn.experts, "prefetch_enabled", False):
            import expert_prefetch
            expert_prefetch.before_attention(
                self, x, input_ids, residual, post_mix, res_mix, attn_pre
            )
        ...
        # Preserve attention, router and shared-expert GEMM shapes. Only the
        # SSD-backed routed experts may omit final rows without consumers.
        self.ffn.experts.output_rows = output_rows
        try:
            x = self.ffn(x, input_ids)
        finally:
            self.ffn.experts.output_rows = None
...
class DeepseekV4Model(nn.Module, EagleModelMixin):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self._dspark_aux_only = spec_config is not None and spec_config.method == "dspark"
    ...
    def forward(
        ...
        final_output_rows: torch.Tensor | None = None,
        ...
        engram_hashes: torch.Tensor | None = None
        engram_mask: torch.Tensor | None = None
        stager=getattr(self,'disk_stager',None)
        if stager is not None:
            engram_hashes=stager.hashes[:input_ids.shape[0]]
            engram_mask=stager.keep[:input_ids.shape[0]]
        elif (
        ...
        if final_output_rows is not None:
            if (self.use_sequence_parallel or self.start_layer != 0
                    or self.end_layer != self.config.num_hidden_layers
                    or self.end_layer in self.aux_hidden_state_layers
                    or not self._dspark_aux_only or not self.aux_hidden_state_layers):
                final_output_rows = None
        for idx, layer in enumerate(
            islice(self.layers, self.start_layer, self.end_layer),
            start=self.start_layer,
        ):
            selected = final_output_rows if idx == self.end_layer - 1 else None
            reference = None
            if selected is not None:
                from decoder_rows import validate_enabled
                if validate_enabled():
                    reference = tuple(t.index_select(0, selected).clone() for t in layer(
                        hidden_states, positions, input_ids, pre_mix, post_mix,
            ...
            if selected is not None:
                from decoder_rows import record_selection
                record_selection(full_num_tokens, selected.numel(), reference,
                    tuple(t.index_select(0, selected) for t in
                          (hidden_states, residual, post_mix, res_mix, pre_mix)))
