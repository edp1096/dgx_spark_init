...
```bash
cd ~/workspace/dgx_spark_init/compose_yaml/sglang_gemma4
docker compose up -d --build
docker logs -f sglang-gemma4
```
...
초기화하지 않으므로 compose에서 `SGLANG_MAX_THINK_TOKENS=32768`을 설정한다.
요청별 값이 이 상한을 덮어쓰며, 32768은 이 프로필의 context 상한이라 사실상
무제한 기본값으로 동작한다.
