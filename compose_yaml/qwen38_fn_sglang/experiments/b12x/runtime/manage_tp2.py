...
def name(rank):return os.environ.get('QWEN_TP2_HEAD_CONTAINER' if rank==0 else 'QWEN_TP2_WORKER_CONTAINER',f'sglang-qwen38-b12x-probe-{rank}')
def node_root(rank):return ROOT if rank==0 else WORKER_ROOT
IMAGE=os.environ.get('QWEN_TP2_IMAGE','dgx-sglang-qwen38-fn:b12x-probe')
...
def env_for(rank,context,token):
    ...
    for key in ('QWEN_TP2_HEAD','QWEN_TP2_API_PORT','QWEN_TP2_DIST_PORT','SGLANG_B12X_PROBE_MOE','SGLANG_B12X_PROBE_HEAD'):
        if key in os.environ:env[key]=os.environ[key]
    env.update(QWEN_TP2_CONTAINER=name(rank),QWEN_TP2_HF_CACHE=os.environ.get('HF_CACHE' if rank==0 else 'WORKER_HF_CACHE',str(Path.home()/'.cache/huggingface')),QWEN_TP2_CACHE_ROOT=os.environ.get('QWEN_TP2_HEAD_CACHE' if rank==0 else 'QWEN_TP2_WORKER_CACHE',str(Path.home()/'.local/share/sparktalk/cache/sglang-flash-next-tp2')),QWEN_TP2_IF=os.environ.get('HEAD_NCCL_IF' if rank==0 else 'WORKER_NCCL_IF','enp1s0f1np1'),QWEN_TP2_HCA=os.environ.get('HEAD_NCCL_HCA' if rank==0 else 'WORKER_NCCL_HCA','rocep1s0f1'),QWEN_TP2_BIND=os.environ.get('QWEN_TP2_BIND','0.0.0.0'),QWEN_TP2_MODEL=os.environ.get('QWEN_TP2_MODEL','qwen3.8-flash-next'))
    return env
