...
p=argparse.ArgumentParser();p.add_argument('--context',type=int,required=True);p.add_argument('--output',required=True);p.add_argument('--url',default='http://127.0.0.1:8012');a=p.parse_args()
from transformers import AutoTokenizer
from runtime.tp2.entrypoint import MODEL
...
base={'context':a.context,'input_tokens':len(ids),'needle_positions':positions,'expected':expected,'kv_dtype':'bfloat16','time_started':time.time()}
out.write_text(json.dumps(base,indent=2)+'\n')
request={'input_ids':ids,'sampling_params':{'temperature':0,'max_new_tokens':256},'stream':True}
start=time.monotonic();first=None;last=None
try:
 req=urllib.request.Request(a.url+'/generate',data=json.dumps(request).encode(),headers={'Content-Type':'application/json'})
 with urllib.request.urlopen(req,timeout=7200) as response:
  for line in response:
   ...
   if row.get('text') and first is None:first=time.monotonic()-start
 elapsed=time.monotonic()-start
 assert last is not None,'No response'
 meta=last.get('meta_info',{});text=last.get('text','')
