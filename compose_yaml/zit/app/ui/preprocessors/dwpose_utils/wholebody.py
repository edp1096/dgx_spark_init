"""Wholebody pose estimation using cv2.dnn ONNX backend.

Adapted from VideoX-Fun/comfyui/annotator/dwpose_utils/wholebody.py
Uses cv2.dnn.readNetFromONNX exclusively -- no onnxruntime dependency.
"""
...
class Wholebody:
    """DWPose wholebody estimator using cv2.dnn backend.

    Args:
        onnx_det: Path to YOLOX detection ONNX model file.
        onnx_pose: Path to DWPose keypoint ONNX model file.
    """

    def __init__(self, onnx_det: str, onnx_pose: str):
        # Always loads to CPU via cv2.dnn (no GPU build required)
        backend = cv2.dnn.DNN_BACKEND_OPENCV
        target = cv2.dnn.DNN_TARGET_CPU

        self.session_det = cv2.dnn.readNetFromONNX(onnx_det)
        self.session_det.setPreferableBackend(backend)
        self.session_det.setPreferableTarget(target)

        self.session_pose = cv2.dnn.readNetFromONNX(onnx_pose)
        self.session_pose.setPreferableBackend(backend)
        self.session_pose.setPreferableTarget(target)
