...
`POST /v1/documents`: `format`, `title`, `sections`(heading/paragraphs/table/images 또는 blocks) 또는 `slides`(title/bullets 또는 blocks), XLSX는 `sheets`(name/columns/rows). `GET /health`: 지원 형식·작업 상태. 작업은 동시에 1개, 최대 90초입니다. 파일명은 ASCII이며 응답의 `files`에 base64 파일이 포함됩니다.

본문·표·이미지·차트·서식·다양한 배치를 지원합니다. 전체 기능과 입력 예시는 [문서 확장 규격](docs/RICH_DOCUMENTS.md)을 참고하세요. 모든 형식에 세션 첨부 이미지를 넣을 수 있고, PPTX는 오디오·영상 첨부도 지원합니다. SparkTalk 도구는 `images: [{image_id, width_cm, caption}]`으로 기존 이미지 ID를 참조하며, 서버가 PNG로 변환해 전달합니다(최대 6개, 변환 후 합계 8 MiB). 외부 URL·임의 HTML·스크립트는 받지 않습니다. PDF는 Office 원본과 줄바꿈·페이지 배치가 다를 수 있습니다.
...
수식은 기본 연산과 SUM/AVERAGE/COUNT/COUNTA/MIN/MAX/IF/AND/OR/NOT/ROUND/ROUNDUP/ROUNDDOWN/ABS/COUNTIF/SUMIF/IFERROR를 지원합니다. 제공된 셀만 참조하며 순환 참조와 계산 오류는 거부합니다. Excelize로 계산한 결과를 원래 수식·서식과 함께 저장합니다. 차트·피벗·셀 서식·조건부 서식·입력 검증 등을 지원합니다. 매크로는 지원하지 않습니다. Excel 자체에서의 호환성은 별도 확인 대상입니다.
...
```sh
docker build -t sparktalk-extra-documents:0.5.0 .
