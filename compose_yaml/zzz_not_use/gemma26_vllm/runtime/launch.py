...
def command(env):
 ...
 if family not in ('ornith35','gemma26'):raise ValueError('Unsupported model family')
 ...
 if family=='ornith35':
  ...
  text['rope_parameters']['full_attention']={**text['rope_parameters']['full_attention'],'factor':factor}
  reasoning,tools='gemma4','gemma4'
 ...
 args=['vllm','serve',str(view),'--served-model-name',env['SERVED_MODEL_NAME'],
       ...
       '--moe-backend',env.get('MOE_BACKEND','b12x' if family=='ornith35' else 'marlin'),
 ...
 if tokens:
  ...
  if family=='ornith35':
   ...
   draft_path=Path(env.get('DRAFT_MODEL_PATH','/hf/google/gemma-4-26B-A4B-it-assistant'))
   if not (draft_path/'config.json').is_file():raise ValueError(f'Gemma assistant is missing: {draft_path}. Set MTP_TOKENS=0 to use the main model alone.')
   draft_config=json.loads((draft_path/'config.json').read_text())
   draft_config['text_config']['max_position_embeddings']=context
   draft_config['text_config']['rope_parameters']['full_attention']['factor']=factor
   draft_root=Path(env.get('RUNTIME_DRAFT_VIEW_ROOT','/root/.cache/vllm/runtime-draft-models'))
   spec['model']=str(config_view(draft_path,draft_config,draft_root))
...
def draft_vocab_path(family,model_path,vocab_root):
 prefix={'ornith35':'ornith','gemma26':'gemma'}[family]
